...
function cnotemodal() {
    Swal.fire({
        title: '備註',
        html:
            '<p>家有兒童：勾選此項目可配送適合兒童的蔬果<br>（如：少籽、易咀嚼）</p>' +
            '<p>辛香料：勾選此項目可隨機配送少量辛香料<br>（如：蔥、薑、蒜）</p>',
        icon: 'info',
        showCancelButton: false,
        confirmButtonColor: '#80c97e',
        confirmButtonText: '確定'
...
function addInputRow() {
    if ($('#inputContainer .input-row').length < maxInputs) {
        ...
        newInput.append('<input type="text" class="form-control my-1 input-with-tag" placeholder="請輸入您的評語，或拖拉上方訂購品項綠色標籤進行評價"><br>');
