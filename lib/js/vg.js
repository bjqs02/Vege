...
$(document).ready(function () {
  ...
  function addProduct(addButton) {
    var container = addButton.closest(".middle_Con"); // 获取最近的 .middle_Con 元素
    var section = addButton.closest("section"); // 获取最近的 .middle_Con 元素
    var sumValue = container.find(".sum").text(); // 获取 .sum 元素的文本
    var productName = section.find("h3").text();
    var commodity = container.find(".commodity option:selected").text();
    var doublePriceText = container.find(".price1").text();
    var familyPriceText = container.find(".price3").text();
    var wowprice = container.find(".price4").text();
    var price = 0;
    let Img = "";
    ...
    // 如果commodity不等于上方情况，将其设置为$(".fv_con .buyfv").text();
    if (!commodity) {
  ...
  $(".addCard").on("click", function () {
    ...
    // 获取产品信息
    var container = $(this).closest(".middle_Con");
    var section = $(this).closest("section");
    var sumValue = container.find(".sum").text();
    var productName = section.find("h3").text();
    var commodity = container.find(".commodity option:selected").text();
    var newProductId = `product-${productCount - 1}`;
    var addButton = $(this); // 获取点击的按钮
  ...
  $(".products").on("click", ".remove-button", function () {
