    ...
    .then((data) => {
      ...
      data.forEach((item) => {
        ...
        <span class="product-details">
            ...
            <p class="m-1">${item.c_option}</p>
            <p class="m-1"><span class='comcon'></span></p>
            <p class="m-1"><span>數量:</span><span>${item.quantity}</span></p>
            </div>
                <input
                type="hidden"
                name="item-price"
...
window.onload = function () {
  ...
  navUser.onclick = function () {
    if (userToken) {
      ...
      document.cookie = `location=${hereLocation
  ...
  checkEmail.onclick = function (e) {
    ...
    var now = new Date();
    var expirationTime = new Date(now.getTime() + 1 * 60 * 60 * 1000);
    document.cookie = `email=${data.mail
      }; expires=${expirationTime.toUTCString()}; Domain=127.0.0.1; path=/; port=2407`;
...
$(document).ready(function () {
  ...
  function addProduct(addButton) {
    ...
    console.log("commodity selected" + commodity);
    console.log("doublePriceText" + doublePriceText);
    console.log("familyPriceText" + familyPriceText);
    ...
    if (
      ["雙人箱(1~2人)", "雙人箱(30day)", "雙人箱(60day)"].includes(commodity)
    ) {
      price = doublePriceText;
    } else if (productName === "自選蔬果箱") {
      price = wowprice;
    } else {
