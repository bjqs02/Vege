...
$(".bcl,.btn-close").on("click", function () {
  $(".fv_box").addClass("shrinkAndMoveAnimation2");

  setTimeout(function () {
    $(".fv_box, .black").hide(); // 使用fadeOut()来隐藏元素
    $(".fv_box").removeClass("shrinkAndMoveAnimation2");
  }, 600);
});

$(".btn-close").on("click", function () {
  $(".modal-content").addClass("shrinkAndMoveAnimation");

  setTimeout(function () {
    $(".modal").modal("hide"); // 直接隐藏模态框
    showModalOnPage();
    $(".modal-content").removeClass("shrinkAndMoveAnimation");
  }, 600);
});

// chage 選擇蔬果 按鈕 點擊顯示水果頁面
$(".chage").on("click", function () {
  $(".fv_box, .black").fadeIn(); // 使用fadeIn()来显示元素
...
$(".mo_con .goSure").on("click", function () {
  ...
  if (selectedTotalPay < minTotalPay || selectedTotalPay > maxTotalPay) {
    ...
    container.find(".price5").text(parseInt(wowprice, 10) * 4);
    ...
    $("body").removeClass("body-no-scroll");
    $("body").css("overflow", "auto");
    wowprice3.text(parseInt(wowprice, 10) + 200);
  }
});
    ...
    if (xhr.readyState === XMLHttpRequest.DONE) {
      if (xhr.status === 200) {
        ...
        for (let i = 0; i < 7; i++) {
          ...
          $(".smtext .productOrigin")[i].textContent = newData.Origin;
...
function fetchImagesFromDatabase() {
  fetch("/api/images")
    ...
    .then((image) => {
      let imgElements = document.querySelectorAll(".ch");
