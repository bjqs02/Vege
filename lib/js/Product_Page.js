...
function set(responseData) {
  ...
  $(".mo_con .goSure").on("click", function () {
    ...
    var buyfvElement = $(".fv_con .buyfv2");
...
function refreshData() {
  ...
  xhr.onreadystatechange = function () {
    if (xhr.readyState === XMLHttpRequest.DONE) {
      if (xhr.status === 200) {
        const newData = JSON.parse(xhr.responseText)[0];
        const productInfoElement = document.getElementById("productInfo");
        const saveInfoElement = document.getElementById("saveInfo");
        const noteInfoElement = document.getElementById("noteInfo");

        productInfoElement.textContent = newData.product;
        saveInfoElement.textContent = newData.save;
        noteInfoElement.textContent = newData.note;
      } else {
